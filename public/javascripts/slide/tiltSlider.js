...
;( function( window ) {
	...
	function extend( a, b ) {
        ...
        function fitWidth( viewWidth, items) {
               if ( viewWidth > 1150) {
                      for(let i=0; i < items.length; i++) {
                          items[i].style.width = "1150px";
                          items[i].style.marginLeft = parseInt((viewWidth - 1150)/2)+"px";
                      }
                } else {
                      for(let i=0; i < items.length; i++) {
	...
	function TiltSlider( el, options ) {
		...
		this.items = this.el.querySelector( 'ol.slides' ).children;
                ...
                fitWidth(window.innerWidth, this.items);         
		// total items
	...
	TiltSlider.prototype._init = function(tiltSliderX) {
		var self = tiltSliderX;
		setInterval(function() {		
		console.log(self);
		var idx;
		idx = self.current;
		console.log(idx);
		if (idx == (self.itemsCount - 1)) {
			self._showItem(0)
		} else {

			self._showItem(idx+1);
		}
		}, 3900);
	}
	...
	TiltSlider.prototype._showItem = function( pos ) {
		...
		var self = this,
			// the current item
			currentItemA = this.items[ this.current ];
		...
		currentItemA.setAttribute( 'data-effect-out', outEffect );
		...
		var cntAnims = 0,
			...
			animElemsCurrentCount = currentItemA.querySelector( '.tiltview' ).children.length, 
			...
			onEndAnimationCurrentItem = function() {
				++animEndCurrentCnt;
				var endFn = function() {
					classie.removeClass( currentItemA, 'hidea' );
					classie.removeClass( currentItemA, 'current' );
					isFinished();
				};
				...
				if( !isSupported ) {
					...
					currentItemA.removeEventListener( animEndEventName, onEndAnimationCurrentItem );
					endFn();
				}
			},
		...
		classie.addClass( currentItemA, 'hidea' );
		classie.addClass( nextItem, 'show' );

		if( isSupported ) {
			currentItemA.addEventListener( animEndEventName, onEndAnimationCurrentItem );
        ...
        window.addEventListener('resize', function(event) {
                ...
                var slideshowElm =  window.document.getElementById("slideshow");
                var items = slideshowElm.querySelector( 'ol.slides' ).children;
                fitWidth(viewWidth, items);
/*                if ( viewWidth > 1150) {
                      for(let i=0; i < items.length; i++) {
                          items[i].style.width = "1150px";
                          items[i].style.marginLeft = parseInt((viewWidth - 1150)/2)+"px";
